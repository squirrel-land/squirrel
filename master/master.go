...
func (master *Master) packetHandler(myIdentity int) {
	...
	for {
		...
		if master.addressPool.IsBroadcast(bufferedPacket.Packet.NextHop) || bufferedPacket.Packet.NextHop.IsMulticast() {
			recipients := master.september.SendBroadcast(myIdentity, 8*len(bufferedPacket.Packet.Packet), underlying)
			...
			if err == nil && master.clients[nextHopId] != nil && master.september.SendUnicast(myIdentity, nextHopId, 8*len(bufferedPacket.Packet.Packet)) {
