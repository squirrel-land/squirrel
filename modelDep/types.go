...
type September interface {
	...
	// Used when a unicast packet as large as size is sent from source(identity) to destination(identity).
	...
	// Used when a broadcast packet as large as size is sent from source(identity).
